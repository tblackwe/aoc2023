...
class Day10
  ...
  track_loop = Array.new(maze.size) { Array.new(maze[0].size) { ' ' } }
  step_count = 0
  loop do
    step_count += 1
    track_loop[current_node[:y]][current_node[:x]] = '*'
  ...
  2.times do
    track_loop.each_with_index do |y, y_index|
      temp = y.each_index.select { |i| y[i] == '*' }
      track_loop[y_index].fill(0, temp.first) { |_i| '-' }
      track_loop[y_index].fill(temp.last.nil? ? nil : temp.last + 1) { |_i| '-' }
    end
    track_loop = track_loop.transpose.map(&:reverse)
  end

  track_loop2 = track_loop.map { |line| line.reduce(:+) }
  pp track_loop2.map { |line| line.count(' ') }.sum
  # pp track_loop2
end
